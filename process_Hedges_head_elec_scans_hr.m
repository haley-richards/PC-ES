...
% PROCESS_HEDGES_HEAD_ELEC_SCANS_HR MATLAB code for process_Hedges_head_elec_scans_hr.fig
%      PROCESS_HEDGES_HEAD_ELEC_SCANS_HR, by itself, creates a new PROCESS_HEDGES_HEAD_ELEC_SCANS_HR or raises the existing
%      singleton*.
%
%      H = PROCESS_HEDGES_HEAD_ELEC_SCANS_HR returns the handle to a new PROCESS_HEDGES_HEAD_ELEC_SCANS_HR or the handle to
%      the existing singleton*.
%
%      PROCESS_HEDGES_HEAD_ELEC_SCANS_HR('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PROCESS_HEDGES_HEAD_ELEC_SCANS_HR.M with the given input arguments.
%
%      PROCESS_HEDGES_HEAD_ELEC_SCANS_HR('Property','Value',...) creates a new PROCESS_HEDGES_HEAD_ELEC_SCANS_HR or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before process_Hedges_head_elec_scans_hr_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to process_Hedges_head_elec_scans_hr_Fcn via varargin.
%
...
% Edit the above text to modify the response to help process_Hedges_head_elec_scans_hr
...
gui_State = struct('gui_Name',       mfilename, ...
    ...
    'gui_OpeningFcn', @process_Hedges_head_elec_scans_hr_OpeningFcn, ...
    'gui_OutputFcn',  @process_Hedges_head_elec_scans_hr_OutputFcn, ...
    'gui_LayoutFcn',  [] , ...
    'gui_Callback',   []);
...
% --- Executes just before process_Hedges_head_elec_scans_hr is made visible.
function process_Hedges_head_elec_scans_hr_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to process_Hedges_head_elec_scans_hr (see VARARGIN)

% Choose default command line output for process_Hedges_head_elec_scans_hr
handles.output = hObject;
%add correct paths and home directory
cd('S:\eit\Haley\GitHub_Standalone_Code\final_gui_code_and_mfiles')
addpath(genpath(cd))
%load homepage
...
function varargout = process_Hedges_head_elec_scans_hr_OutputFcn(hObject, eventdata, handles)
...
handles.cropped   = 0;
handles.plotplane = 0;
handles.ini_rz    = 0;         % str2double(get(handles.ini_rz,'String'))*pi/180;
handles.ini_rx    = 90*pi/180; % str2double(get(handles.ini_rx,'String'))*pi/180;
handles.org_x     = 0;       
handles.org_y     = 0;       
handles.org_z     = 0;
handles.zpl_x1    = -0.1;
...
for n = 1:2
    ...
    %----------------------------------------------------------------------
    % Plot only a reduced version of the scan
    %     handles_tmp    = handles;
    %     is             = randsample( size(locs,1), min([100*1e3,0.2*size(locs,1)]) );
    %     handles_tmp.colobj = pointCloud(locs(is,:), 'Color', cols(is,:));
    %     update_ptcld_plot(hObject,handles_tmp)
    %     guidata(hObject, handles); % Make sure the h0bject is updated with the original scan
    update_ptcld_plot(hObject,handles)
