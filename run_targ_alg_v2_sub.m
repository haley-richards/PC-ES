function [newtdats, targ_label, colobj_out] = run_targ_alg_v2_sub(colobj,pts0, dbg_flg, colobj0)
% clear
% clc
% close all
% load testdat
...
[cents, targ_label] = find_targs_hr(img_m3,dbg_flg);
%-------------------------
...
if size(cents, 1) > 0
    ...
    for n = 1:size(cents,1)
      ...
      [locs_sort, idx]=sort(((locs(:,1) - cent_m(n,1)).^2 + (locs(:,2) - cent_m(n,2)).^2), 'ascend');
       cols_out(idx(1:n_colpts), :)=repmat([0, 250, 0], n_colpts, 1); 
        newtdats(n,:) = locs0((j),:);
    end
