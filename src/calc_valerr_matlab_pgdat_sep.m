...
%
%--------------------------------------------------------------------------
function [rms_sep_with_filter,mdn_sep_with_filter, max_sep_with_filter, n_sep_els] = calc_valerr_matlab_pgdat_sep(pgdat,allscans,outthresh,dbg_flg)

%--------------------------------------------------------------------------
% Loop through all the individual scans
rms_sep_with_filter = zeros(1,length(allscans));
mdn_sep_with_filter = zeros(1,length(allscans));
max_sep_with_filter = zeros(1,length(allscans));
n_sep_els           = zeros(1,length(allscans));
for n = 1:length(allscans)
    tdat_mrg = allscans(n).tdat(1:256, :);
    ...
    %----------------------------------------------------------------------
    % Removing the outlier detection... I don't see we can know the
    % separated scans are outliers if there is a distance threshold for
    % choosing to label the electrodes
    % %     %----------------------------------------------------------------------
    ...
    %----------------------------------------------------------------------
    % Return the
    rms_sep_with_filter(n) =   sqrt(1/n_el*sum( diff(good_els).^2 ) );
    mdn_sep_with_filter(n) = median(diff(good_els));
    max_sep_with_filter(n) = max(diff(good_els));
    n_sep_els(n)           = n_el;
end
